...
function ActiveRoom({ roomId, userId, role, onLeave }: { roomId: string, userId: string, role: 'speaker' | 'listener', onLeave: () => void }) {
    const { isConnected, error, consumers, isMicOn, leaveRoom } = useAudioRoom(roomId, userId, role);
    ...
    return (
        <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center p-8 font-sans">
            ...
            <div className="w-full max-w-4xl grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                ...
                {/* Local User */}
                <div className="relative group rounded-2xl bg-gray-900 border border-gray-800 aspect-square flex flex-col items-center justify-center overflow-hidden transition-all hover:border-purple-500/50">
                    <div className="absolute inset-0 bg-gradient-to-br from-purple-600/10 to-transparent"></div>
                    <div className={`w-24 h-24 rounded-full flex items-center justify-center text-3xl font-bold shadow-2xl ${isMicOn ? 'bg-purple-600' : 'bg-gray-800'}`}>
                        {userId.charAt(0).toUpperCase()}
                    </div>
                    <p className="mt-4 font-medium z-10">{userId} (You)</p>
                    <p className="text-sm text-gray-400 z-10">{isMicOn ? 'Mic On' : 'Mic Off'}</p>
                </div>

                {/* Remote Users */}
                {consumers.map(c => (
                    <div key={c.id} className="relative rounded-2xl bg-gray-900 border border-gray-800 aspect-square flex flex-col items-center justify-center overflow-hidden transition-all">
                        <div className="w-24 h-24 rounded-full bg-blue-600 flex items-center justify-center text-3xl font-bold shadow-2xl">
                            <span className="animate-pulse absolute w-32 h-32 bg-blue-500/20 rounded-full"></span>
                            🔊
                        </div>
                        <p className="mt-4 font-medium z-10">{c.userId || 'Remote Voice'}</p>
                        <AudioConsumer track={c.track} />
                    </div>
                ))}
            </div>
        </div>
